	...
	// if (window.innerWidth <= 1386) {
	 const menuInterBody = document.querySelector('.filters'); {
			 if (document.querySelector(".filters-button")) {
					document.addEventListener("click", function (e) {
					 // if (bodyLockStatus && e.target.closest('.interactive__icon--hover')) {
					 // 	 bodyLockToggle();
						 ...
						 } else if (!menuInterBody.contains(e.target)) {
								menuCloseFilters();
						 }
					});
			 };
...
// }
export function menuCloseFilters() {
	// bodyUnlock();
...
// 	 const menuSearchBody = document.querySelector('.header__actions'); {
...
// 						 }
// 						 else if (!menuSearchBody.contains(e.target)) {
// 							menuCloseSearch();
// 						 }
// 					});
// 			 };
// 		}
// 	}
// export function menuCloseSearch() {
// 	// bodyUnlock();
// 	document.documentElement.classList.remove("lock")
...
export function menuSearch() {
	if (document.querySelector(".button-search")) {
			document.addEventListener("click", function (e) {
					// Проверяем, если клик произошел на элементе с классом 'button-search'
					if (e.target.closest('.button-search')) {
							document.documentElement.classList.add("lock-search");
							document.documentElement.classList.add("show-search");
					}
					// Проверяем, если клик произошел на элементе с классом 'button-search-close'
