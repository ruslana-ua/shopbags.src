...
export function asideScroll() {
	...
	// if (windowWidth < 1386 && windowWidth > 767.98) {
	// if (windowWidth < 1386) {
		addWindowScrollEvent = true;
			...
			document.addEventListener("windowScroll", function (e) {
				...
				if (scrollTop >= startPoint) {
					...
					if (headerShow) {
						if (scrollTop > scrollDirection) {
							// downscroll code
							header.classList.contains('_aside-show') ? header.classList.remove('_aside-show') : null;
						} else {
							// upscroll code
							!header.classList.contains('_header-show') ? header.classList.add('_aside-show') : null;
						}
						timer = setTimeout(() => {
