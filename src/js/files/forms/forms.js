...
export function formFieldsInit(options = { viewPass: false, autoHeight: false }) {
	...
	// 			// targetElement.parentElement.classList.remove('_form-valid');
	// 		}
...
export let formValidate = {
	...
	validateInput(formRequiredItem) {
		...
		if (formRequiredItem.dataset.required === "email") {
		...
		else if (formRequiredItem.dataset.required === "comment") {
			// formRequiredItem.value = formRequiredItem.value.replace(" ", "");
			if (this.commentTest(formRequiredItem)) {
				this.addError(formRequiredItem);
				error++;
			} else {
				this.removeError(formRequiredItem);
			...
			for (let i = 0; i < radioInputs.length; i++) {
					if (radioInputs[i].checked) {
							radioChecked = true;
							break; // Прекращаем проверку, как только найден отмеченный радио-инпут
					}
			}
			if (!radioChecked) {
					this.addError(formRequiredItem);
  ...
  //   if (errorElement && formRequiredItem.parentElement.contains(errorElement)) {
  //       formRequiredItem.parentElement.removeChild(errorElement);
  //   }
	// },
	...
	removeError(formRequiredItem) {
		...
		var errorElement = formRequiredItem.parentElement.querySelector('.form__error');
		if (errorElement && errorElement.parentElement === formRequiredItem.parentElement) {
				formRequiredItem.parentElement.removeChild(errorElement);
		}
	},
...
// 	validateInput(formRequiredItem) {
// 		let error = 0;
// 		if (formRequiredItem.type === "radio" && !this.radioGroupChecked(formRequiredItem)) {
// 				this.addError(formRequiredItem);
// 				error++;
// 		} else {
...
// 		formRequiredItem.parentElement.classList.remove('_form-error');
// 		let inputError = formRequiredItem.parentElement.querySelector('.form__error');
// 		if (inputError && formRequiredItem.parentElement.contains(inputError)) {
...
// else 	if (formRequiredItem.type === "radio" && !this.radioGroupChecked(formRequiredItem)) {
// 	this.addError(formRequiredItem);
// 	error++;
// } else {
// 	if (!formRequiredItem.value.trim()) {
