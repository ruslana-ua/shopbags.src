...
class SelectConstructor {
	constructor(props, data = null) {
		...
		this.selectClasses = {
			...
			classFormValid: "_form-valid", // Вибрано правильний пункт
		}
		this._this = this;
    ...
    if (!selectOptions.classList.contains('_slide')) {
        ...
        // Оновлюємо заголовок селекту
        this.setSelectTitleValue(selectItem, originalSelect);
        // Додаємо клас _form-valid якщо є вибране значення
        if (originalSelect.value) {
            selectItem.classList.add(this.selectClasses.classFormValid);
        } else {
            selectItem.classList.remove(this.selectClasses.classFormValid);
        }
        // Викликаємо реакцію на зміну селекту
        this.setSelectChange(originalSelect);
    }
    ...
    // Додаємо клас _form-valid якщо є вибране значення
    if (originalSelect.value) {
        originalSelect.parentElement.classList.add(this.selectClasses.classFormValid);
    } else {
        originalSelect.parentElement.classList.remove(this.selectClasses.classFormValid);
    }
    this.setSelectChange(originalSelect);
}
